...
async def delete_users_except_admin(usuario: dict = Depends(get_current_user)):
    """
    Endpoint para eliminar todos los usuarios excepto admin@gmail.com
    Solo puede ser ejecutado por admin@gmail.com
    """
    # Verificar que el usuario actual es admin
    if usuario.get("correo") != "admin@gmail.com":
        raise HTTPException(
            status_code=403, 
            detail="Solo el usuario admin puede ejecutar esta operación"
        )
    ...
    try:
        ...
        # Verificar que admin@gmail.com existe
        admin_user = await usuarios_collection.find_one({"correo": "admin@gmail.com"})
        if not admin_user:
            raise HTTPException(
                status_code=404, 
                detail="No se encontró el usuario admin@gmail.com"
            )
        
        # Contar usuarios antes de eliminar
        total_users = await usuarios_collection.count_documents({})
        
        # Eliminar todos los usuarios excepto admin@gmail.com
        result = await usuarios_collection.delete_many({
            "correo": {"$ne": "admin@gmail.com"}
        })
        
        deleted_count = result.deleted_count
        remaining_users = await usuarios_collection.count_documents({})
...
async def change_admin_password(
    ...
    """
    Endpoint para cambiar la contraseña del usuario admin
    Solo puede ser ejecutado por admin@gmail.com
    """
    # Verificar que el usuario actual es admin
    if usuario.get("correo") != "admin@gmail.com":
    ...
    try:
        ...
        nueva_contraseña_hash = hashear_contraseña(new_password)
        ...
        if result.modified_count == 0:
            raise HTTPException(
                status_code=404, 
                detail="No se pudo actualizar la contraseña del usuario admin"
            )
...
@router.put("/admin/reset-password-emergency")
async def reset_admin_password_emergency():
    """
    Endpoint de emergencia para cambiar la contraseña del admin a 'salchipapa'
    ⚠️ SOLO PARA USO TEMPORAL - ELIMINAR DESPUÉS DE USAR
    """
    try:
        from app.core.auth import hashear_contraseña
        ...
        nueva_contraseña_hash = hashear_contraseña(nueva_contraseña)
        ...
        return {
            "message": "Contraseña de emergencia establecida exitosamente",
            "usuario": "admin@gmail.com",
            "nueva_contraseña": nueva_contraseña,
            "warning": "⚠️ ELIMINAR ESTE ENDPOINT DESPUÉS DE USAR"
        }
...
@router.patch("/usuarios/{id}/permisos")
async def actualizar_permisos_usuario(
    id: str, 
    permisos: List[str] = Body(...), 
    usuario: dict = Depends(get_current_user)
):
    """Actualizar permisos de un usuario específico - Solo admin"""
    if usuario.get("correo") != "admin@gmail.com":
        raise HTTPException(status_code=403, detail="Solo el usuario admin puede modificar permisos")
    ...
    if result.modified_count == 0:
        raise HTTPException(status_code=500, detail="No se pudieron actualizar los permisos")
...
async def obtener_permisos_usuario(id: str, usuario: dict = Depends(get_current_user)):
    """Obtener permisos de un usuario específico"""
    if usuario.get("correo") != "admin@gmail.com":
        raise HTTPException(status_code=403, detail="Solo el usuario admin puede ver permisos")
    ...
    usuarios_collection = get_collection("USUARIOS")
    usuario_obj = await usuarios_collection.find_one(
        {"_id": oid}, 
        {"permisos": 1, "correo": 1, "_id": 1}
    )
    ...
    usuario_obj["_id"] = str(usuario_obj["_id"])
    return {"permisos": usuario_obj.get("permisos", [])}
